...
class RuntimeTemplate {
	...
	importStatement({
		...
		const proposalAsyncSemantics =
			deferImportOption && deferImportOption.asyncModule === "proposal";
		...
		if (defer) {
			...
			importContent = `/* deferred harmony import */ ${optDeclaration}${importVar} = ${
				RuntimeGlobals.makeDeferredNamespaceObject
			}(${moduleId}, 0, ${JSON.stringify(
				Array.from(innerAsyncDependencies, mod => mod.id)
			)}${innerAsyncDependencies.size > 0 ? ", __webpack_handle_async_dependencies__" : ""});\n`;
		} else {
	...
	exportFromImport({
		...
		defer = defer && !moduleGraph.isAsync(module);
		const deferOpt = this.compilation.options.experiments.deferImport;
		...
		if (exportName.length > 0) {
			...
			const access = `${importVar}${
				defer && deferOpt && deferOpt.asyncModule !== "proposal" ? ".a" : ""
