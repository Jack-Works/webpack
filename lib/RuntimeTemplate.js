...
class RuntimeTemplate {
	...
	importStatement({
		...
		const deferImportOption = this.compilation.options.experiments.deferImport;
		const proposalAsyncSemantics = deferImportOption && deferImportOption.asyncModule === "proposal";

		let innerAsyncDependencies = new Set();

		if (proposalAsyncSemantics) {
			defer = defer && !module.buildMeta.async;
			...
			if (defer) {
				...
				(function gatherInnerAsyncDependencies(mod) {
					...
					seen.add(mod);
					if (mod.buildMeta.async) {
						innerAsyncDependencies.add(mod);
					} else {
						for (const dep of mod.dependencies) {
							gatherInnerAsyncDependencies(moduleGraph.getModule(dep));
						}
					}
				})(module)
			}
		} else {
			defer = defer && !moduleGraph.isAsync(module);
		}
		...
		let importContent;
		if (defer) {
			if (innerAsyncDependencies.size > 0) {
				importContent = Template.asString([
					`/* deferred harmony import */ ${optDeclaration}${importVar} = Promise.all([`,
					Template.indent(
						Array.from(innerAsyncDependencies, mod =>
							`/* deferred harmony import async dependency */ __webpack_require__(${mod.id}),`
						)
					),
					"]).then(function () {",
					Template.indent([
						// we ignore the first call to the get trap, since it's the promise reading
						// .then when returning this proxy from this .then callback.
						`return ${getProxyDeferredModule(this,	moduleId, /* skipFirstGet */ true)};`
					]),
					"});\n"
				])
			} else {
				importContent = Template.asString([
					`/* deferred harmony import */ ${optDeclaration}${importVar} = {`,
					Template.indent([`a: ${getProxyDeferredModule(this, moduleId)}`]),
					`};\n`
				]);
			}
		} else {
			importContent = `/* harmony import */ ${optDeclaration}${importVar} = __webpack_require__(${moduleId});\n`;
		}
