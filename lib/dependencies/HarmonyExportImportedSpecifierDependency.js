...
/** @typedef {import("../Dependency").HarmonyDependencyCheckMode} HarmonyDependencyCheckMode */
...
class HarmonyExportImportedSpecifierDependency extends HarmonyImportDependency {
	 ...
	 */
	getWarnings(moduleGraph, checkMode) {
		const exportsPresence = this._getEffectiveExportPresenceLevel(moduleGraph);
		return this._getLinkingErrorOrWarnings(moduleGraph, {
			// these two options never emit warnings, only error.
			deferModuleMode: false,
			useSyncAssertion: false,
			strictExportPresence: exportsPresence === ExportPresenceModes.WARN
		});
	}
	...
	getErrors(moduleGraph, checkMode) {
		const exportsPresence = this._getEffectiveExportPresenceLevel(moduleGraph);
		return this._getLinkingErrorOrWarnings(moduleGraph, {
			deferModuleMode: checkMode && checkMode.deferModuleMode,
			useSyncAssertion: checkMode && checkMode.useSyncAssertion,
			strictExportPresence: exportsPresence === ExportPresenceModes.ERROR
		});
	}

	/**
	 * @param {ModuleGraph} moduleGraph module graph
	 * @param {HarmonyDependencyCheckMode=} checkMode check mode
	 * @returns {WebpackError[] | undefined} errors
	 */
	_getLinkingErrorOrWarnings(moduleGraph, checkMode) {
		...
		if (!checkMode.strictExportPresence) {
			return errors;
		}
...
HarmonyExportImportedSpecifierDependency.Template = class HarmonyExportImportedSpecifierDependencyTemplate extends (
	...
	getReexportDeferredNamespaceObjectFragments(
		...
		map.set(
			...
			}(${moduleId}, ${getMakeDeferredNamespaceModeFromExportsType(
				exportsType
			)}))`
		);
